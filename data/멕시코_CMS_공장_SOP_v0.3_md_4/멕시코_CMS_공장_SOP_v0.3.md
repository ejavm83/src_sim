...
전선 제조가 처음이라면 2장(전체 구성)과 4장(공정 상세)의 서술만 따라가도 전체 흐름을 이해할 수 있고, 모르는 용어는 부록의 삽화 해설(10.1, 그림 16종)과 용어집(10.2)에서 찾을 수 있습니다. 현장 운영 관점에서는 4장의 표준 작업 절차와 6장(설비·교체·캘린더)이 기준이 됩니다. 시뮬레이션 엔지니어라면 5장(라우팅·소요 시간), 7장(제약 조건), 8장(파라미터 도출)과 별첨 워크북을 중심으로 읽으면 됩니다.
...
- 별첨 C: 멕시코_CMS_설비_장비_목록_v0.3 — 공정별 설비·장비 목록(대수·공유·파라미터·예시 사진 링크)
- 별첨 D: 멕시코_CMS_용어_삽화_해설집_v0.3 — 일반인용 용어 삽화 16종(부록 10.1과 동일)
...
선속은 케이블이 설비를 통과하는 속도로, 원본 자료의 'm/m'은 분당 미터(m/min)를 뜻합니다. 보빈은 케이블을 감는 큰 릴이며 지름을 Φ(파이, mm)로 부릅니다 — 이 공장에는 630Φ, 800Φ, 1050Φ, 1250Φ가 쓰입니다. 캐리어는 태신선이 쓰는 1톤 단위 운반 용기, 다발은 보빈 없이 감은 코일 묶음, 파렛트는 출하 적재 단위입니다. 'AL 16스퀘어'처럼 스퀘어(㎟)는 도체 단면적을 뜻합니다. 상세한 환산은 부록 10.3에, 보빈·캐리어·다발·파렛트의 생김새는 부록 삽화 14에 있습니다.
...
자동차 전선의 구조는 단순합니다(삽화 1·2). 가운데에 전기가 흐르는 금속 심(도체)이 있고, 그 위에 전기가 새지 않게 하는 플라스틱 옷(절연체)을 입힙니다. 전기차용 고전압 전선은 여기에 전자파를 막는 금속 갑옷(편조 차폐)과 바깥 보호 겉옷(시스)을 한 겹씩 더 입힙니다.
...
### 2.6 메트로 맵 ② — 위치·설비 노선도 (역 = 엑셀 위치 코드 + 설비, 같은 위치를 지나야 환승역)

2.5의 지도는 '공정' 단위라서 같은 공정 역에 각 라인의 전용 설비와 공유 설비가 함께 놓여 있었습니다. 그러나 원본 엑셀의 '위치' 열을 보면 각 라인은 공정마다 정해진 위치의 설비를 쓰고, 그 위치가 라인마다 다르거나 같습니다. Cu44·Cu19·AL16은 절연 압출 E12·E13, 조사 E15·E16, 재권취 E23, 검사 E24·E25처럼 E계열 위치를 함께 지나고(진짜 공유), Cu44·Cu19만 태신선 E2·E3, 멀티신선 E4, 연선 E6을 함께 씁니다. 반면 AL16의 도체 공정은 A계열(A2·A3 멀티신선, A4~A8 집합)로 따로 있고, 실리콘 라인은 S계열(S2·S3 압출, S4·S5 편조, S6·S7 테이핑, S9 검사)과 R1·W1을 씁니다. 아래 지도는 역을 이 '위치 코드 + 설비(대수)'로 정의해, 같은 위치를 두 라인 이상이 지나는 곳만 굵은 환승역으로, 나머지는 각 노선의 전용 역으로 그린 것입니다. 노란 사각은 적재 장소·물류 지점(버퍼)입니다. 시뮬레이션의 자원 풀은 이 지도의 굵은 역 하나하나에, 버퍼는 노란 사각 하나하나에 대응합니다.

![fig_metro_loc.png](images/fig_metro_loc.png)


- 환승역(굵은 테두리) — R3 입고 램프(Cu44·Cu19·AL16), E2·E3 태신선기 1대·E4 멀티신선기 1대·E6 연선기 6대(Cu44·Cu19), E12·E13 절연압출기 2대·E15·E16 조사기 1대·E24·E25 검사(Cu44·Cu19·AL16), E23 재권취기 왼쪽 1050Φ 2대(Cu44·AL16), W2 완제품 창고(전 라인). 역 아래 색점이 그 위치를 지나는 라인입니다.
- 전용 역(얇은 테두리) — E7·E8 집합기 10대·E9 튜블러 1대·E23 재권취기 오른쪽 1250Φ 2대(Cu19), A2·A3 멀티신선기·A4~A8 집합기(AL16, ⚠**대수 확인 필요**), S2·S3 실리콘압출기 2대·S9 검사(실리콘), E18 편조기 21대·E19 테이핑기 4대·E21·E22 시스압출기 2대(Cu44 차폐 SKU).
- 위치 코드가 알려 주는 것: 실리콘 편조(S4·S5)·테이핑(S6·S7)은 Cu44의 E18·E19와 위치가 달라 ⚠**별도 설비로 해석(대수 TBD, 질문 #1)**했고, 실리콘 시스는 절연 압출과 같은 S2·S3이라 ⚠**같은 실리콘압출기를 다시 쓰는 것으로 해석(질문 #18)**했습니다. E23 재권취는 같은 위치에 왼쪽 2대(1050Φ)와 오른쪽 2대(1250Φ)가 있어 두 역으로 나눴습니다.
- 버퍼(노란 사각) — E1·A1·W1→S1(원자재 적재), E10 AMR 도킹·E11 파이 적재(절연 투입 전, 세 라인 공용), E14 절연 적재, E17 조사 적재, F1→S8 AMR 도킹(실리콘). 점선은 Cu44 차폐 SKU 전용 순환 지선이며 ⚠**조사 ②는 같은 조사기 E15·E16(질문 #19)**으로 되돌아옵니다.
...
도체 공정은 굵은 금속을 가늘게 늘리는 신선(2.1 태신선, 2.2 멀티신선)과 가늘어진 선을 꼬아 묶는 연선(2.3 집합, 2.4 연선, 2.5 튜블러)으로 이루어집니다(삽화 2·3·4·5). 왜 가늘게 만든 뒤 다시 꼬는가 하면, 굵은 통짜 금속선은 뻣뻣해서 자동차 안의 좁은 경로에 배선할 수 없기 때문입니다. 머리카락 굵기의 소선 수십 가닥을 꼬면 같은 단면적이라도 유연하고 끊어지지 않는 도체가 됩니다. 실리콘 라인은 도체를 원자재로 구매하므로 이 대공정을 거치지 않고, AL16은 2mm 선재로 입고되어 2.2부터 시작합니다.


#### 2.1 태신선 — 8mm를 2mm로 늘리는 1차 인발

【무엇을 하는 공정인가】 8mm 구리 로드를 다이스(구멍이 뚫린 초경 금형) 여러 개에 차례로 통과시키며 2mm까지 늘리는 공정입니다(삽화 3·4). 금속은 한 번에 크게 줄일 수 없어서 단계적으로 가늘게 만드는데, 그 첫 번째 큰 단계가 태신선입니다. 굵기가 4분의 1이 되는 동안 길이는 약 16배로 늘어납니다. 태신선기는 1대뿐이며 Cu44와 Cu19가 함께 씁니다 [도체정리].
...
【무엇을 하는 공정인가】 가는 소선 여러 가닥을 처음으로 꼬아 다발을 만드는 공정입니다(삽화 5). 꼬임 간격(피치)을 일정하게 유지하며 소선들을 한 몸으로 묶습니다. Cu19 라인은 집합기 10대를 쓰고 [도체정리], AL16 라인은 더블 집합기 1대와 싱글 집합기 12대를 조합해 씁니다.
...
【무엇을 하는 공정인가】 튜블러연선기(태연선기)는 큰 통이 회전하면서 여러 가닥을 한 번에 꼬는 설비로(삽화 6), Cu19 전용이며 1대가 있습니다 [도체정리]. 집합만 마친 선과 연선까지 마친 선을 함께 투입해 최종 19복합 도체를 완성합니다. 구 시트에 적힌 '630Φ 보빈 12개 + 800Φ 보빈 1개'는 ⚠**설비에 장착되는 보빈 배열(중심 1 + 외곽 12 = 19복합 구조)을 가리키는 것으로 해석됩니다**.
...
【무엇을 하는 공정인가】 압출기는(삽화 7) 도체를 풀어 주는 페이오프, 컴파운드를 150~250℃로 녹여 도체 표면에 균일하게 입히는 실린더·헤드, 입힌 절연체를 굳히는 냉각 수조, 완성품을 감는 테이크업으로 구성되며, 설비 간 속도 차이를 흡수하는 저선기가 사이에 들어갑니다(기준공정위키). 색은 뉴트럴 컴파운드에 착색 원료(CMB)를 배합해 내며, 색 변경 시 CMB만 바꾸면 됩니다. 절연압출기는 2대를 Cu44·Cu19·AL16 세 라인이 공유합니다 [도체정리].
...
【무엇을 하는 공정인가】 빛의 속도에 가깝게 가속한 전자빔을 절연체에 쏘면(삽화 8·9) 분자 결합이 일부 끊어졌다가 서로 그물처럼 다시 연결됩니다(가교). 이렇게 하면 저렴한 원료로도 내열 등급을 크게 올릴 수 있습니다. 방사선을 차폐하는 쉴드룸이라는 밀폐 시설 안에서 진행되고, 케이블이 미로형 통로를 지나며 처리되므로 가동 중 내부 접근이 불가능합니다(기준공정위키). 조사기는 1대이며 Cu44·Cu19·AL16이 공유하고, ⚠**시스 조사(5.2)까지 같은 1대가 겸하는 것으로 기재되어 있습니다(질문 #19)**.
...
【무엇을 하는 공정인가】 주석 도금된 가는 금속 실 수십 가닥을 베틀처럼 서로 교차시키며 케이블 표면에 90% 이상의 밀도로 갑옷을 짜는 공정입니다(삽화 10). 촘촘히 땋아야 차폐 성능이 나오기 때문에 선속이 1.5m/min로, 가장 빠른 멀티신선(1,200m/min)의 800분의 1입니다. 이 극단적인 느림 때문에 편조기는 21대나 설치되며 [도체정리], 그래도 공장 전체의 병목 후보 1순위입니다.
...
【무엇을 하는 공정인가】 구리 또는 알루미늄 마일라 테이프를 일정한 겹침률로 나선형으로 감아(삽화 11) 차폐를 보강하는 공정입니다. 편조보다 훨씬 빠르며 테이핑기는 4대입니다 [도체정리].
...
【무엇을 하는 공정인가】 수천~수만 미터짜리 공장 로트를 고객 주문 길이로 잘라 작은 보빈이나 다발로 다시 감는 공정입니다(삽화 12). 여기서 '공장의 로트'가 '판매 단위'로 바뀌므로, 시뮬레이션의 로트 변환 로직이 반드시 필요한 지점입니다. 재권취기는 보빈 규격별로 나뉘어 1050Φ용 2대(Cu44·AL16 공유)와 1250Φ용 2대(Cu19)가 있습니다 [도체정리].
...
【무엇을 하는 공정인가】 재권취 완료품을 일렬로 배치해 놓고 작업자가 외관과 이상 유무를 육안으로 확인하는 공정입니다 [도체정리]. 대표 품질 지표는 편심(삽화 13) — 단면에서 도체가 절연체의 정중앙에서 벗어난 정도 — 이며, 편심이 크면 압출기 헤드의 센터 볼트를 수동 조정해 보정합니다(기준공정위키). 합격품은 파렛트 단위로 적재됩니다.
...
| 편조기(Cu) — E18 | 21 | CU44 (SIL은 S4·S5 별도 위치) | 1h | 총 능력 21대×2,160m/일 ≈ 45,360m/일[산출] · 도체정리§4-7 |
| 테이핑기 — E19 | 4 | CU44 (SIL은 S6·S7 별도 위치) | 0.5h | 도체정리§4-8 |
| 시스압출기 — E21·E22 | 2 | CU44 (SIL 시스는 S2·S3 실리콘압출기 재사용으로 해석) | 1h | 도체정리§4-9 |
...
| ◆SIL 편조기(S4·S5)·테이핑기(S6·S7) | TBD | SIL 전용(위치 코드상 Cu44 설비와 별도) | 0.5~1h | 대수 미확인 — 질문 #1 · 실리콘!D14~D18 |
...
| --- | --- | --- | --- | --- | --- |
| 1 | 설비 대수 | Cu 라인 설비 대수는 신규 문서로 확정(태신선1·멀티신선1·집합10·연선6·튜블러1·절연압출2·조사1·편조21·테이핑4·시스압출2·재권취 1050Φ 2/1250Φ 2). 남은 확인: ① AL16 도체 설비(멀티신선·집합 더블/싱글) 대수 ② 실리콘 라인 편조기(S4·S5)·테이핑기(S6·S7) 대수 — 위치 코드가 Cu44의 E18·E19와 달라 별도 설비로 해석되나 대수 미기재(실리콘 AL 350,000m/월이면 편조 약 7대분 필요[산출]) | 상 | 일부 해소(v0.2) | 도체정리 전체, 실리콘!N14 |
...
| 18 | 실리콘 | CU 실리콘의 월 물량(AL은 25t/월=350,000m). 시스 압출 위치 코드가 절연 압출과 동일(S2·S3)하여 같은 실리콘압출기를 다시 쓰는 것으로 해석 — 맞는지, 그렇다면 절연·시스 작업의 배정 규칙 | 중 | 미해결(해석 보강) | 실리콘!C2·E19 |
...
### 10.1 그림으로 보는 핵심 용어 — 삽화 16종

전선 제조가 처음인 독자를 위해 본문에 나오는 생소한 용어를 그림으로 풀었습니다. 그림은 실물 사진이 아니라 원리를 보여 주는 도식이며, 각 그림 아래에 쉬운 말로 보충 설명과 관련 용어를 달았습니다. 본문 4장의 각 공정에서 '(삽화 N)'으로 이 그림을 가리킵니다.

![fig_t01_cable.png](images/fig_t01_cable.png)

**삽화 1  전선의 단면 구조 — 도체·절연·편조·시스**

전선을 잘라 단면을 보면 양파처럼 겹이 있습니다. 가운데 구리 소선 다발이 도체(전기가 흐르는 심), 그 위 파란 층이 절연체(전기가 새지 않게 막는 플라스틱)입니다. 일반전장(LV) 전선은 이 두 겹으로 끝나지만, 전기차용 고전압(HV) 전선은 전자파를 가두는 금속 실 갑옷(편조 차폐)과 외부 충격·마모를 막는 겉옷(시스)을 한 겹씩 더 입습니다. 이 문서의 공정은 결국 이 겹을 안쪽부터 하나씩 만들어 붙이는 순서입니다.

*관련 용어: 도체 · 절연체 · 편조 차폐 · 시스 · LV · HV*

![fig_t02_two_stages.png](images/fig_t02_two_stages.png)

**삽화 2  전선은 두 단계로 만든다 — 도체 공정과 피복 공정**

공장의 일은 크게 둘로 나뉩니다. 앞쪽 도체 공정은 굵은 금속 막대(로드)를 다이스로 가늘게 늘리고(신선) 가는 선 수십 가닥을 꼬아(연선) 도체를 만드는 단계이고, 뒤쪽 피복 공정은 그 도체에 녹인 플라스틱을 입히고(압출) 전자빔으로 열에 강하게 만든 뒤(조사) 고객 길이로 감아(재권취) 검사·출하하는 단계입니다. 본문 4장의 공정 1~6이 이 순서를 그대로 따릅니다.

*관련 용어: 로드 · 신선 · 연선 · 압출 · 조사 · 재권취*

![fig_t03_drawing.png](images/fig_t03_drawing.png)

**삽화 3  신선(인발)과 다이스 — 가늘고 길게 늘리기**

신선(伸線)은 금속선을 다이스—가운데 구멍이 뚫린 아주 단단한 금형—에 통과시켜 잡아당기는 가공입니다. 구멍이 출구로 갈수록 좁아지므로 선은 가늘어지고 그만큼 길어집니다(굵기가 1/4이 되면 길이는 16배). 금속은 한 번에 크게 줄일 수 없어 다이스를 여러 개 차례로 지나며 조금씩 줄이는데, 8mm 로드를 2mm로 만드는 첫 단계를 태신선, 2mm를 0.3mm 안팎으로 여러 가닥 동시에 뽑는 단계를 멀티신선이라 부릅니다.

*관련 용어: 신선 · 인발 · 다이스 · 태신선 · 멀티신선 · 인취*

![fig_t04_sizes.png](images/fig_t04_sizes.png)

**삽화 4  얼마나 가늘어지는가 — 로드·선재·소선 굵기 비교**

실제 비율로 그린 그림입니다. 8mm 구리 로드는 손가락 굵기이지만, 태신선을 거치면 2mm(연필심보다 조금 굵은 정도)가 되고, 멀티신선을 거치면 0.29~0.32mm—샤프심(0.5mm)의 절반 남짓, 머리카락(약 0.08mm)의 서너 배—인 소선이 됩니다. 이렇게 가는 소선을 수십 가닥 꼬는 이유는 같은 단면적이라도 훨씬 유연해서 자동차 안의 좁은 경로에 배선할 수 있기 때문입니다.

*관련 용어: 로드 · 선재 · 소선*

![fig_t05_strand_code.png](images/fig_t05_strand_code.png)

**삽화 5  집합·연선·복합과 자재 코드 — 19/9/0.315 읽는 법**

소선을 처음 꼬아 다발을 만드는 것이 집합, 다발을 다시 크게 꼬는 것이 연선·복합입니다. 도체 이름 19/9/0.315 Cu는 이 구조를 그대로 숫자로 적은 것으로 '지름 0.315mm 구리 소선 9가닥짜리 다발을 19개(가운데 1 + 안쪽 6 + 바깥 12) 모아 꼰 도체'라는 뜻입니다. Cu44의 44/0.29는 소선 44가닥을 한 번에 꼰 단순 구조입니다. 옆에서 보면 선이 나선을 그리며 감기는데, 한 바퀴 꼬이는 데 걸리는 길이가 피치이고 꼬임 품질의 기본 관리값입니다.

*관련 용어: 집합 · 연선 · 복합 · 자재 코드 · 피치*

![fig_t06_tubular.png](images/fig_t06_tubular.png)

**삽화 6  튜블러 연선기 — 통이 통째로 돌며 한 번에 꼰다**

튜블러 연선기(태연선기)는 긴 통 안에 보빈들이 들어 있고 통 자체가 회전하면서 각 보빈에서 나온 선을 앞쪽 다이스에서 한 번에 꼬는 설비입니다. Cu19 라인에서는 집합만 마친 선 1보빈과 연선까지 마친 선 1보빈을 함께 넣어 최종 19복합 도체(20,000m 다발 4개)를 만들며, 두 로트가 모두 도착해야 시작할 수 있는 '병합' 공정입니다.

*관련 용어: 튜블러 · 태연선기 · 병합*

![fig_t07_extruder.png](images/fig_t07_extruder.png)

**삽화 7  압출기 — 페이오프·저선기·실린더·헤드·냉각 수조·테이크업**

압출기는 다섯 부분으로 이루어집니다. 페이오프가 도체를 풀어 공급하고, 저선기가 앞뒤 설비의 속도 차이를 흡수하며(보빈 교체 시간을 벌어 줌), 호퍼로 들어간 컴파운드 알갱이(색을 내는 CMB 포함)가 실린더 안에서 150~250℃로 녹아 헤드에서 도체 표면에 균일하게 입혀지고, 냉각 수조에서 식어 굳은 뒤 테이크업이 보빈에 감습니다. 색을 바꿀 때는 컴파운드는 그대로 두고 CMB만 교체합니다. 시스 압출도 원료만 다를 뿐 같은 구성입니다.

*관련 용어: 압출 · 페이오프 · 저선기 · 실린더·헤드 · 냉각 수조 · 테이크업 · 컴파운드 · CMB*

![fig_t08_ebeam.png](images/fig_t08_ebeam.png)

**삽화 8  조사(照射) 설비 — 쉴드룸 안의 전자빔**

조사(照射)는 전자를 빛의 속도의 90~96%까지 가속해 케이블에 쏘는 공정입니다. 방사선이 밖으로 새지 않도록 두꺼운 콘크리트 벽(쉴드룸)으로 둘러싸고 출입구를 미로처럼 꺾어 두며, 케이블은 롤러를 따라 여러 바퀴 돌면서 전자빔을 골고루 맞습니다. 가동 중에는 안을 들여다보거나 들어갈 수 없습니다. 이 공장에서는 조사기 1대를 여러 라인이 함께 씁니다.

*관련 용어: 조사 · 전자빔 · 가속기 · 쉴드룸 · 미로형 통로*

![fig_t09_crosslink.png](images/fig_t09_crosslink.png)

**삽화 9  가교(架橋) — 분자 사슬을 그물처럼 엮기**

절연 플라스틱은 긴 분자 사슬이 따로따로 놓여 있어 열을 받으면 쉽게 물러집니다. 전자빔을 쏘면 사슬의 결합이 일부 끊어졌다가 서로 이어지며 그물 구조가 되는데, 이것이 가교(架橋)입니다. 가교되면 열에 강해져 저렴한 원료로도 높은 내열 등급을 얻을 수 있고, 대신 조금 뻣뻣해집니다. 실리콘은 압출기 안의 열만으로 스스로 가교되기 때문에 실리콘 라인에는 조사 공정이 없습니다.

*관련 용어: 가교 · 내열 등급 · 실리콘 압출*

![fig_t10_braid.png](images/fig_t10_braid.png)

**삽화 10  편조 — 금속 실로 갑옷 짜기 (가장 느린 공정)**

편조는 주석 도금된 가는 금속 실(편조사)을 감은 캐리어 수십 개가 케이블 둘레를 두 줄로 서로 반대 방향으로 돌면서 실을 위아래로 교차시켜, 케이블 위에 90% 이상 밀도의 금속 갑옷을 짜는 차폐 공정입니다. 촘촘히 짜야 하므로 선속이 1.5m/min로 멀티신선(1,200m/min)의 800분의 1이며, 그래서 편조기가 21대나 있어도 공장 병목 1순위입니다.

*관련 용어: 편조 · 편조사 · 차폐 · 밀도*

![fig_t11_tape.png](images/fig_t11_tape.png)

**삽화 11  테이핑 — 금속 테이프를 나선형으로 겹쳐 감기**

테이핑은 구리 또는 알루미늄 마일라(알루미늄을 입힌 필름) 테이프를 케이블에 나선형으로 감는 공정입니다. 앞 바퀴와 일정 폭을 겹치게(겹침률) 감아 빈틈이 없도록 하며, 편조 위를 덮어 고정하고 차폐를 보강합니다. 편조보다 훨씬 빨라 20m/min로 진행됩니다.

*관련 용어: 테이핑 · 마일라 테이프 · 겹침률*

![fig_t12_rewind.png](images/fig_t12_rewind.png)

**삽화 12  재권취 — 고객 길이로 잘라 다시 감기**

재권취는 수천~수만 미터짜리 공장 로트를 고객이 주문한 길이로 잘라 작은 보빈이나 다발에 다시 감는 공정입니다. Cu44는 12,000m 보빈이 610m 보빈 19개로, Cu19는 5,000m가 100m 다발 50개로 나뉘며, 여기서 '공장의 로트'가 '판매 단위'로 바뀝니다. 설정 길이에 도달하면 자동으로 멈추고 절단·라벨을 붙입니다.

*관련 용어: 재권취 · 로트 · 판매 단위*

![fig_t13_eccentric.png](images/fig_t13_eccentric.png)

**삽화 13  편심(偏心) — 도체가 가운데서 벗어난 정도**

편심(偏心)은 단면에서 도체가 절연체의 정중앙에서 벗어난 정도입니다. 도체가 한쪽으로 치우치면 그쪽 절연이 얇아져 전기가 새거나 파괴될 위험이 커지므로 대표 검사 항목이 되며, 현재는 작업자가 육안으로 확인합니다. 편심이 크면 압출기 헤드의 센터 볼트를 상하좌우로 돌려 도체가 가운데를 지나도록 조정합니다.

*관련 용어: 편심 · 검사 · 센터 볼트*

![fig_t14_units.png](images/fig_t14_units.png)

**삽화 14  재료를 나르는 단위 — 보빈(Φ)·캐리어·다발·파렛트**

재료는 공정마다 다른 단위로 감기고 옮겨집니다. 보빈은 양옆에 원판(플랜지)이 달린 릴로, Φ(파이)는 그 원판의 지름이며 이 공장은 630·800·1050·1250Φ를 씁니다. 캐리어는 태신선이 2mm 선재를 담아 내는 1톤 용기(약 38,125m), 다발(코일)은 보빈 없이 둥글게 감은 묶음으로 Cu19 완제품이 100m 다발로 나갑니다. 파렛트는 출하 적재 단위로 Cu44는 18보빈(9개×2단), Cu19는 45다발이 차야 완성됩니다.

*관련 용어: 보빈 · Φ(파이) · 캐리어 · 다발 · 파렛트*

![fig_t15_transport.png](images/fig_t15_transport.png)

**삽화 15  공장 안에서 나르는 것들 — AMR·지게차·대차·계근대**

공장 안 운반은 네 가지가 나눠 맡습니다. AMR은 바닥을 스스로 달리며 길을 찾고 장애물을 피하는 자율주행 운반 로봇으로 창고와 공정 구역 사이의 장거리 정형 반송을 맡고(5km/h), 지게차는 무거운 보빈을 공정 사이로 옮기며 Cu 로드 하차에는 4톤 지게차가 쓰입니다(5km/h). 대차는 사람이 미는 근거리 수레이고, 계근대는 트럭이 통째로 올라가 무게를 재는 대형 저울로 입고 검수에 쓰입니다.

*관련 용어: AMR · 지게차 · 대차 · 계근대*

![fig_t16_sim_concepts.png](images/fig_t16_sim_concepts.png)

**삽화 16  시뮬레이션의 눈으로 본 공장 — 버퍼·공유 설비·로트 변환**

시뮬레이션은 공장을 세 가지 눈으로 봅니다. 첫째, 공정과 공정 사이의 적재 장소는 재고가 줄을 서는 버퍼(대기 행렬)입니다. 둘째, 여러 라인이 함께 쓰는 설비(멀티신선기·연선기·절연압출기·조사기·재권취기)는 지하철 환승역과 같아서 한 라인이 쓰는 동안 다른 라인이 기다리며—이것이 경합이고 병목이 생기는 지점입니다. 셋째, 공정마다 로트 단위가 바뀌어 하나가 여럿으로 나뉘거나(분할: 연선·절연·재권취) 여럿이 하나로 합쳐집니다(병합: 튜블러의 2보빈 동시 투입).

*관련 용어: 버퍼 · 대기 행렬 · 공유 설비 · 경합 · 병목 · 로트 변환*


### 10.2 용어집

본문에 나온 용어를 재료, 도체 공정, 피복·차폐 공정, 설비·물류, 제품·품질, 시뮬레이션의 여섯 범주로 정리했습니다. 그림으로 먼저 보려면 10.1의 삽화를 참조하십시오.
...
| v0.3 | 2026-08-14 | 서술 중심 전면 개편 — 공정 순번 체계(공정 N/N.M), 전체 구성·연관 관계 장, 설비 환승 메트로 맵, 단계별·누적 시간(타임라인), 시뮬레이션 제약 조건 장(7장) 신설, 신뢰도 등급 폐지(확인 필요 주황 표시), 전체 용어집 별첨, 일반인용 용어 삽화 16종(10.1) 추가, 위치·설비 메트로 맵(2.6) 추가 | 공정 표준화 TF |
